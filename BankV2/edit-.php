...
// issivedam balanso kintamaji kad butu paprasciau dirbt
$balance = (int)$account['regbalance'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    //ar 2 skaicia po kablelio max
    if($_POST['regbalance']*100 % intval($_POST['regbalance']*100) != 0 ){
        $_SESSION['transaction']= '<span style="color: crimson; "> Accepted only numbers higher than 0 and 2 decimals</span>';
                header('Location: http://localhost/egprojektas/BankV2/edit-.php?regaccno='.$id);
                die;
    }
    $accounts = array_map(function ($c) use ($id) {
        if ($c['regaccno'] == $id) {
            if ((int)$c['regbalance']-(int)$_POST['regbalance']<0){
                $_SESSION['transaction']= '<span style="color: crimson; "> The Balance Is Too Low </span>';
                header('Location: http://localhost/egprojektas/BankV2/edit-.php?regaccno='.$id);
                die;
            }
